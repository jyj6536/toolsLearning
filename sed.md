...
以 books.txt 为例，如果某一行匹配模式 Paulo，则在该行的前面打印连字符（-），否则打印该行本身：
...
类似于 **p** 命令，我们有 **P** 命令打印由 **N** 创建的多行的模式空间中的第一部分（直到嵌入的换行符）。
...
**n** 命令清空模式缓冲区，将下一行放入模式缓冲区，并对其应用命令。

现在，假定 **n** 之前有三条 SED 命令，**n** 之后有两条 SED 命令。
